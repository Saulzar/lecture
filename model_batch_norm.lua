...
-- stage 2 : mean suppresion -> filter bank -> non linear -> max pooling
...
model:add(nn.SpatialBatchNormalization(features[2]))
model:add(nn.SpatialConvolutionMM(features[2], features[3], 5, 5)) -- output size: 1x1 (linear layer)
model:add(nn.ReLU(true))


model:add(nn.SpatialBatchNormalization(features[3]))
